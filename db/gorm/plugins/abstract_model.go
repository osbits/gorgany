...
func (thiz ExtendedModelProcessor) AddModelTypeAfterInsert(db *gorm.DB) {
	...
	rCurrentModel := util.IndirectType(reflect.TypeOf(model))
	...
	for _, abstractModel := range abstractModels {
		...
		query := fmt.Sprintf("UPDATE %s SET %s = '%s' WHERE %s", tableName, db2.StructModelColumn, rCurrentModel.Name(), strings.Join(conds, ","))
...
func (thiz ExtendedModelProcessor) AddModelTypeToWhere(db *gorm.DB) {
	...
	db.Where("? = ?", db2.StructModelColumn, rModel.Name())
}
